# AegisOps — 4-Minute Demo Script & Run Guide (record-ready)
...
> "This is **AegisOps** — an autonomous SRE that works the entire 3am on-call loop. When a production alert fires, six specialized AI agents triage it, diagnose the logs, **read the Grafana dashboard with vision**, find the bad deploy, propose a fix, and — with one human approval — remediate and write the postmortem. It runs on **Google ADK**, **Gemini 3.5 Flash and Pro on Vertex AI**, **Firestore**, **Pub/Sub**, and **Cloud Run**. Here's a live incident."
...
> "Now the governance gate. AegisOps **will not touch production on its own** — it proposes a **rollback to v2.4.0**, low-risk and reversible, and **halts for a human**. The destructive action is never something the model can trigger itself."
...
> "And the payoff: **AegisOps learns.** Every resolved incident is written back to memory — so when this fingerprint returns, the Memory agent **recalls it** and hands responders the known fix instantly. It gets smarter every time it's used."
...
### 3:55–4:00 — Close
> "**AegisOps** — a real, governed, **learning** multi-agent SRE, live on Google Cloud. Thanks for watching."
...
- [ ] Project named **AegisOps** + tagline (below)
...
### Tagline
> *AegisOps: an autonomous, human-governed multi-agent SRE that diagnoses incidents with multimodal Gemini, learns from every resolution, and runs entirely on Google Cloud.*
